...
class TestMnemosyneAPIIntegration:
    ...
    def test_dynamic_ast_injection_endpoint(self, client):
        ...
        import os
        os.environ["SOLOMON_INTERNAL_AUTH_KEY"] = "test_internal_key"
        headers = {"Authorization": f"Bearer test_internal_key"}
        response = client.post("/api/mnemosyne/ast-inject", data=json.dumps(payload), content_type="application/json", headers=headers)
        assert response.status_code == 200
        data = response.get_json()
        assert data["status"] == "success"
