    ...
    def _init_db(self):
        ...
        try:
            with conn:
                ...
                # Migration 8: Loki Sports Intelligence Enhancements (Odds Movement, Injuries, Weather)
                if current_v < 8:
                    conn.execute("""
                        CREATE TABLE IF NOT EXISTS loki_line_history (
                            ...
                            movement_type TEXT NOT NULL, -- SHARP_MONEY, PUBLIC_MONEY, INJURY_UPDATE, WEATHER_SHIFT
                            timestamp TEXT NOT NULL
                        );
                    """)
                    conn.execute("""
                        CREATE TABLE IF NOT EXISTS loki_injuries (
                            injury_id TEXT PRIMARY KEY,
                            sport TEXT NOT NULL,
                            team TEXT NOT NULL,
                            player TEXT NOT NULL,
                            status TEXT NOT NULL, -- OUT, QUESTIONABLE, PROBABLE
                            impact_score REAL NOT NULL, -- 0.0 to 1.0 (1.0 = star player out)
                            updated_at TEXT NOT NULL
                        );
                    """)
                    conn.execute("""
                        CREATE TABLE IF NOT EXISTS loki_weather (
                            weather_id TEXT PRIMARY KEY,
                            fixture_id TEXT NOT NULL,
                            condition TEXT NOT NULL, -- CLEAR, RAIN, SNOW, WIND
