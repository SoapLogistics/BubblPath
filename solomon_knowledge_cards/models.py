            ...
            with conn:
                ...
                if current_v < 4:
                    ...
                    conn.execute("""
                        ...
                        VALUES
                        ('gabriel', 'Gabriel', 'COMMAND_CENTER_RELAY', 'LIVE', ?),
                        ('mnemosyne', 'Mnemosyne', 'MEMORY_CONTEXT', 'READ_WRITE', ?),
                        ('prometheus', 'Prometheus', 'BUILD_PLANNER', 'LIVE_PLANNING', ?),
                        ('loki', 'Loki', 'SPORTS_RESEARCH_MODEL', 'LIVE_BETTING', ?);
                    """, (now_str, now_str, now_str, now_str))

                    # Update existing values to their LIVE states if already seeded
                    conn.execute("UPDATE worker_modes SET mode = 'LIVE' WHERE worker_id = 'gabriel'")
                    conn.execute("UPDATE worker_modes SET mode = 'READ_WRITE' WHERE worker_id = 'mnemosyne'")
                    conn.execute("UPDATE worker_modes SET mode = 'LIVE_PLANNING' WHERE worker_id = 'prometheus'")
                    conn.execute("UPDATE worker_modes SET mode = 'LIVE_BETTING' WHERE worker_id = 'loki'")
