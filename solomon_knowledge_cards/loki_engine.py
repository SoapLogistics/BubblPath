...
class LokiEngine:
    ...
    def sync_external_feeds(self, fixtures: List[Dict[str, Any]], conn=None) -> Dict[str, Any]:
        """
        Mocks external data feeds (injuries, weather, line movement) for the active fixtures.
        """
        close_conn = False
        if conn is None:
            conn = self.runtime.db.get_connection()
        ...
        updates = {"injuries": 0, "weather": 0, "line_movements": 0}

        try:
            for f in fixtures:
                    fixture_id = f["fixture_id"]
                    sport = f["sport"]
                    teams = f["fixture"].split(" vs ")
                    if len(teams) != 2: continue
                    home, away = teams[0], teams[1]

                    # 1. Weather Feed (Outdoor sports only)
                    if sport in ["NFL", "Premier League"] and random.random() < 0.2: # 20% chance of weather event
                        conditions = ["RAIN", "SNOW", "WIND", "CLEAR"]
                        cond = random.choice(conditions)
                        impact = 0.0
                        if cond == "RAIN": impact = 0.3
                        elif cond == "SNOW": impact = 0.6
                        elif cond == "WIND": impact = 0.4

                        if impact > 0:
                            wid = str(uuid.uuid4())[:8]
                            conn.execute("""
                                INSERT INTO loki_weather (weather_id, fixture_id, condition, temperature, wind_speed, impact_score, updated_at)
                                VALUES (?, ?, ?, ?, ?, ?, ?)
                            """, (wid, fixture_id, cond, random.uniform(20, 85), random.uniform(0, 30), impact, datetime.utcnow().isoformat()))
                            updates["weather"] += 1

                    # 2. Injury Feed (Randomly affect a star player)
                    if random.random() < 0.15: # 15% chance of significant injury news
                        team = random.choice([home, away])
                        statuses = ["OUT", "QUESTIONABLE"]
                        status = random.choice(statuses)
                        impact = random.uniform(0.5, 1.0) if status == "OUT" else random.uniform(0.2, 0.5)

                        iid = str(uuid.uuid4())[:8]
                        conn.execute("""
                            INSERT INTO loki_injuries (injury_id, sport, team, player, status, impact_score, updated_at)
                            VALUES (?, ?, ?, ?, ?, ?, ?)
                        """, (iid, sport, team, f"{team} Star Player", status, impact, datetime.utcnow().isoformat()))
                        updates["injuries"] += 1

                    # 3. Line Movement History
                    # We will mock a line movement for soft odds in the fixtures
                    if f.get("is_soft_line") and f.get("soft_outcome_index") is not None:
                        idx = f["soft_outcome_index"]
                        outcome = f["outcomes"][idx]
                        current_odds = f["soft_odds"][idx]

                        # Mock an old odds that was worse (sharps bet it down) or better
                        old_odds = current_odds * random.uniform(1.05, 1.15)
                        movement_type = random.choice(["SHARP_MONEY", "INJURY_UPDATE", "WEATHER_SHIFT", "PUBLIC_MONEY"])

                        lid = str(uuid.uuid4())[:8]
                        conn.execute("""
                            INSERT INTO loki_line_history (history_id, fixture_id, market, outcome, old_odds, new_odds, movement_type, timestamp)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """, (lid, fixture_id, f["market"], outcome, old_odds, current_odds, movement_type, datetime.utcnow().isoformat()))
                        updates["line_movements"] += 1
            return updates
        except Exception as e:
            logger.error(f"Failed to sync external feeds: {e}")
            return updates
        finally:
    ...
    def get_active_value_picks(self, conn=None) -> List[Dict[str, Any]]:
        """
        Scours live mock fixtures, runs overround correction, identifies value,
        and incorporates sports intelligence (weather, injuries, line history).
        """
        fixtures = self.generate_fixtures()

        # Trigger mock feeds generation
        ...
        if conn is None:
            ...
            close_conn = True
        try:
            self.sync_external_feeds(fixtures, conn=conn)
            picks = []

            # Fetch active weather and injuries for fast lookup
            weather_data = {}
            cursor = conn.execute("SELECT * FROM loki_weather WHERE updated_at > datetime('now', '-1 day')")
            for row in cursor.fetchall():
                weather_data[row["fixture_id"]] = dict(row)

            injury_data = defaultdict(list)
            cursor = conn.execute("SELECT * FROM loki_injuries WHERE updated_at > datetime('now', '-1 day')")
            for row in cursor.fetchall():
                injury_data[row["team"]].append(dict(row))

            history_data = defaultdict(list)
            cursor = conn.execute("SELECT * FROM loki_line_history WHERE timestamp > datetime('now', '-1 day')")
            for row in cursor.fetchall():
                history_data[row["fixture_id"]].append(dict(row))
            ...
            for f in fixtures:
                ...
                sport = f["sport"]
                teams = f["fixture"].split(" vs ")
                home, away = teams[0], teams[1] if len(teams) == 2 else ("", "")
                ...
                for idx, outcome in enumerate(f["outcomes"]):
                    ...
                    l_sentiment = "NEUTRAL"
                    for h in history_data.get(f["fixture_id"], []):
                        if h["outcome"] == outcome:
                            if h["movement_type"] == "SHARP_MONEY":
                                modifier *= 1.05
                                expected_value *= 1.05
                                l_sentiment = "SHARP_BACKED"
        ...
        try:
            with conn:
                ...
                if active_mode == "LIVE_BETTING":
                    picks = self.get_active_value_picks(conn=conn)
                    ...
                    for pick in picks:
                        ...
                        if current_bankroll >= stake:
                            ...
                            explanation = self.explain_pick(pick)
    ...
    def explain_pick(self, pick: Dict[str, Any]) -> str:
        """
        Generates a natural language explanation for a pick, incorporating odds movement, injuries, weather, and confidence.
        """
        explanation = f"Loki recommends {pick['outcome']} ({pick['market']}) at {pick['odds']} odds with a {pick['expected_value']:.2f} EV.\n"

        explanation += f"- **Quantitative Edge:** The Shin True Probability is {pick['shin_true_prob']:.1%}, beating the implied bookmaker odds. Adjusted Kelly suggests risking {pick['kelly_fraction']:.2%} of bankroll.\n"

        if pick.get('confidence_modifier', 1.0) != 1.0:
            if pick['confidence_modifier'] > 1.0:
                explanation += f"- **Historical Confidence:** System has high historic success in this band (Modifier: {pick['confidence_modifier']:.2f}x).\n"
            else:
                explanation += f"- **Historical Confidence:** System is cautious in this band (Modifier: {pick['confidence_modifier']:.2f}x).\n"

        if pick.get('weather_impact', 0.0) > 0:
            explanation += f"- **Weather Impact:** Adverse weather conditions have heavily factored into this projection (Impact: {pick['weather_impact']:.2f}).\n"

        if pick.get('injury_impact', 0.0) > 0:
            explanation += f"- **Injury Feeds:** Significant player absences or questionable designations factored into the true probability (Impact: {pick['injury_impact']:.2f}).\n"

        if pick.get('line_sentiment') == 'SHARP_BACKED':
            explanation += "- **Line History:** Smart money (sharp action) has recently hit this side, corroborating our predictive models.\n"
