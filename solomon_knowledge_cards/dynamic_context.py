...
class DynamicContextEngine:
    def __init__(self, max_vram_mb: int = 1500):
        ...
        self.topic_index = {} # Phase 32: Hierarchical Chunking
    ...
    def budget_context(self, current_vram_usage: float, messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        # Phase 31: Context Defragmentation
        ...
        if estimated_vram > available_vram:
            messages = self._token_level_pruning(messages) # Phase 33
            total_chars = sum(len(m.get("content", "")) for m in messages)
            estimated_vram = (total_chars * 10) / self.bytes_per_mb
            if estimated_vram > available_vram:
    ...
    def _defragment_system_messages(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for msg in messages:
            if msg.get("role") == "system":
                sys_content.append(msg.get("content", ""))
            else:
                defragged.append(msg)
        if sys_content:
            defragged.insert(0, {"role": "system", "content": " | ".join(sys_content), "importance": 1.0})
        return defragged
    ...
    def _compress_messages(self, messages: List[Dict[str, Any]], target_vram: float) -> List[Dict[str, Any]]:
        ...
        while len(compressed) > 2:
            ...
            for i in range(1, len(compressed)):
                ...
                if imp < min_importance:
                    min_importance, idx_to_remove = imp, i
        ...
        if len(compressed) < len(messages):
            compressed.insert(1, {"role": "system", "content": "[CONTEXT COMPRESSED]", "importance": 0.9})
        return compressed
