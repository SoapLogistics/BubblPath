...
class ModelRouter:
    ...
    # Model specs
    TARGET_MODEL = {
        "name": "High-Precision FP16/INT8 Target Model (70B/32B)",
        "precision": "FP16/INT8",
        "ram_footprint_gb": 14.0,
        "base_latency_ms": 55.0,
        "token_cost_multiplier": 1.0
    }

    QUANTIZED_MODEL = {
        "name": "Ultra-Light Quantized INT4/Ternary Model (2B/1.58b)",
        "precision": "INT4/Ternary",
        "ram_footprint_gb": 0.7,
        "base_latency_ms": 12.0,
        "token_cost_multiplier": 0.05
    }
    ...
    def route_query(self, query: str, threshold: float = 0.15) -> Dict[str, Any]:
        ...
        # Calculate VRAM and cost metrics
        vram_saved_gb = self.TARGET_MODEL["ram_footprint_gb"] - routed_model["ram_footprint_gb"]
        latency_reduction_percent = round(
            ((self.TARGET_MODEL["base_latency_ms"] - routed_model["base_latency_ms"])
             / self.TARGET_MODEL["base_latency_ms"]) * 100.0, 1
        )

        cost_savings_percent = round(
            (1.0 - (routed_model["token_cost_multiplier"] / self.TARGET_MODEL["token_cost_multiplier"])) * 100.0, 1
        )
        ...
        return {
            ...
            "active_ram_footprint_gb": routed_model["ram_footprint_gb"],
            "estimated_latency_ms": routed_model["base_latency_ms"],
            "decision_reason": decision_reason,
            "resource_impact": {
                "vram_saved_gb": round(max(0.0, vram_saved_gb), 2),
                "latency_reduction_percent": max(0.0, latency_reduction_percent),
