...
def bad_request_handler(e):
    ...
    return jsonify({
        "error": "Bad Request",
        "message": getattr(e, "description", "Malformed or invalid request payload.")
    }), 400
...
def health():
    ...
    if not api_key_configured:
        logger.error("Health check failed: OPENAI_API_KEY environment variable is not configured.")
        response_payload["message"] = "Service misconfigured: OpenAI API key is missing."
    else:
...
def chat():
    ...
    if len(user_message) > 4000:
        logger.warning(f"Rejected message exceeding maximum length: {len(user_message)} characters.")
        return jsonify({
            "error": "Bad Request",
    ...
    try:
        ...
        logger.error(f"OpenAI APIStatusError encountered: status_code={e.status_code}, response={e.response}")
        return jsonify({
