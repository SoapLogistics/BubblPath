...
@app.route("/api/mnemosyne/graph/resolve", methods=["POST"])
def resolve_knowledge_graph():
    """
    Resolves the topological graph ordering or reasoning context for a specific target card.
    """
    data = request.json or {}
    card_id = data.get("card_id")
    action = data.get("action", "reason") # 'reason' or 'topology'

    if action == "topology":
        topo = knowledge_graph.resolve_topology()
        return jsonify({
            "status": "success",
            "topological_order": topo
        })
    elif action == "reason" and card_id:
        context = knowledge_graph.reason_over_relationships(card_id)
        formatted = knowledge_graph.format_reasoning_for_llm(context)
        return jsonify({
            "status": "success",
            "context": context,
            "formatted_reasoning": formatted
        })
    else:
        return jsonify({"error": "Missing 'card_id' or invalid 'action'."}), 400
