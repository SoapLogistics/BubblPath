...
# ==========================================
# SOSS PHASE 20: DYNAMIC CONTEXT BUDGETER ROUTING
# ==========================================
...
def evaluate_context_budget():
    ...
    try:
        available_ram_mb = float(data.get("available_ram_mb", 1024.0))
        available_vram_mb = float(data.get("available_vram_mb", 1024.0))
    except (ValueError, TypeError):
        return jsonify({"error": "Invalid 'available_ram_mb' or 'available_vram_mb' parameter, must be a float."}), 400

    history = data.get("prompt_history", [])

    if not isinstance(history, list):
        return jsonify({"error": "'prompt_history' must be a list."}), 400

    report = context_budgeter.evaluate_budget(
    ...
    return jsonify({
        ...
        "recommended_next_step": (
            "RECOMMENDED NEXT STEP:\n"
            "<span style='color: #00E676; font-weight: bold; font-size: 1.25em;'>"
            "Utilize this dynamic sliding context window to feed the central language model prompt, "
            "guaranteeing absolute immunity against OOM (Out-of-Memory) crashes during infinite memory loops!</span>"
        )
    })
