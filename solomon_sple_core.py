...
from solomon_sple_unified_engine import PerpetualLearningEngine

class UnifiedOrchestrator(Orchestrator):
    """
    Extends Orchestrator to use the PerpetualLearningEngine for all tasks.
    """
    def __init__(self):
        super().__init__()
        self.learning_engine = PerpetualLearningEngine()
        logger.info("UnifiedOrchestrator initialized with PerpetualLearningEngine.")

    def process_queue(self):
        """Processes the internal task queue via the unified engine."""
        if not self.task_queue:
            return

        task = self.task_queue.pop(0)
        logger.info(f"Unified processing task: {task['type']}")

        # Route ALL tasks through the single learning pipeline
        result = self.learning_engine.process_learning_event(task['type'], task['payload'])
        logger.info(f"Pipeline Result: {result.get('subsystem_results', {})}")
