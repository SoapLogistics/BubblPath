...
class SolomonMnemosyneDB:
    ...
    def __init__(self, db_path: str = "solomon_mnemosyne_demo.db"):
        ...
        self._init_db()
        # Initialize the native embeddings model
    ...
    def compute_local_embedding_sync(self, text: str) -> List[float]:
        """
        Synchronous worker for semantic embeddings using all-MiniLM-L6-v2.
        """
        if not text.strip():
            # Fallback for empty text (384 dimensions)
            vector = [0.0] * 384
            vector[0] = 1.0
            return vector
        ...
        # Ensure it's returned as List[float] for compatibility
        return [float(x) for x in vector]

    def compute_local_embedding(self, text: str) -> List[float]:
        """
        Submits embedding task to background worker thread pool and awaits result.
        """
        future = self.executor.submit(self.compute_local_embedding_sync, text)
