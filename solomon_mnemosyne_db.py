...
class SolomonMnemosyneDB:
    ...
    def _init_db(self):
        ...
        # 1. Create knowledge_cards table (including embedding, confidence, and validation_state)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS knowledge_cards (
                ...
                validation_state TEXT DEFAULT 'ACTIVE'
            )
        """)
        ...
        if "validation_state" not in columns:
            cursor.execute("ALTER TABLE knowledge_cards ADD COLUMN validation_state TEXT DEFAULT 'ACTIVE'")
    ...
    def upsert_card(self, card_id: str, family: str, focus: str, content: str, validation_state: str = "ACTIVE") -> bool:
        """
        Upserts a SOK card, automatically calculating and caching its local vector embedding.
        Preserves existing confidence scores and validation state on update if already on disk.
        """
        ...
        try:
            # Check if card already exists to preserve confidence and state
            cursor.execute("SELECT confidence, validation_state FROM knowledge_cards WHERE card_id = ?", (card_id,))
            row = cursor.fetchone()
            confidence = row[0] if row else 1.0
            existing_state = row[1] if row else validation_state

            cursor.execute("""
                INSERT INTO knowledge_cards (card_id, family, focus, content, embedding, confidence, validation_state)
                VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    def update_card_validation_state(self, card_id: str, state: str) -> bool:
        """
        Transitions a card through the Review Gate (DRAFT -> REVIEWED -> APPROVED -> ACTIVE).
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT 1 FROM knowledge_cards WHERE card_id = ?", (card_id,))
            if not cursor.fetchone():
                return False
            cursor.execute("""
                UPDATE knowledge_cards SET validation_state = ?
                WHERE card_id = ?
            """, (state, card_id))
            conn.commit()
            return True
    ...
    def semantic_search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        Only returns cards that are in 'ACTIVE' validation state to prevent unapproved card poisoning.
        """
        ...
        results = []
        try:
            cursor.execute("SELECT card_id, family, focus, content, embedding, confidence, validation_state FROM knowledge_cards")
            for row in cursor.fetchall():
                card = dict(row)
                if not card["embedding"]:
                    continue

                # Filter out non-ACTIVE cards to prevent poisoning
                if card.get("validation_state", "ACTIVE") != "ACTIVE":
                    continue
                ...
                results.append({
                    ...
                    "validation_state": card.get("validation_state", "ACTIVE"),
